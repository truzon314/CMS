...
export function SeoSettingsForm({ settings, isSaving, onSave }: SeoSettingsFormProps) {
  ...
  return <div className="flex max-w-2xl flex-col gap-3">
    ...
    <ImagePickerField label="Default Open Graph image" imageUrl={ogUrlOverride ?? currentOgImage?.url ?? ""} onChange={({ url, mediaId }) => { setOgUrlOverride(url); setOgMediaId(mediaId); }} />
